...
# Função para fixar a janela da webcam 
def fix_webcam_position():
    windows = gw.getWindowsWithTitle("Hand Detection")
    if windows:
        window = windows[0]
        screen_width, screen_height = pyautogui.size()
        target_x = screen_width - 720
        target_y = 60
        window.moveTo(target_x, target_y)
    else:
        print("Janela da webcam não encontrada.")
...
while True:
    ...
    fix_webcam_position()  # Mantém a posição fixa da janela
